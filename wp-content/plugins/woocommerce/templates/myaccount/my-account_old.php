...
foreach($pronvalsone as $key => $orproductproq){
	if(isset($getpagarr[$orproductproq->product_id]['stock']) && !empty($getpagarr[$orproductproq->product_id]['stock']) && ($orproductproq->tot_prod_qty>=$getpagarr[$orproductproq->product_id]['stock']) ){//echo $orproductproq->product_id;
		$pagearray[$b]['date'] = date( 'm/d/y', $getpagarr[$orproductproq->product_id]['endate']  );
		$pagearray[$b]['post_title'] = $getpagarr[$orproductproq->product_id]['post_title'];
		$pagearray[$b]['amount'] = $orproductproq->subtotal;
		$pagearray[$b]['stock'] = $orproductproq->tot_prod_qty;
	}
...
if(count($pagearray)>0){
	...
	<tr  class="order">
			...
			<td class="order-status" style="text-align:left; white-space:nowrap;"><span style="color:blue;font-size: 16px;"><?php echo $orproductproqc['post_title']; ?></span> endded successfully (<?php echo $orproductproqc['stock']; ?> shirt<?php if($orproductproqc['stock']>1){echo 's';}else{}?> sold)</td>
