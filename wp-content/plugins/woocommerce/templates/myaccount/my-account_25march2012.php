...
$getpagarr = array();
foreach($orproduct_pro as $orproductpro){
	$getpagarr[$orproductpro->order_id]['buyeremail'] = $orproductpro->buyer_email;
	$getpagarr[$orproductpro->order_id]['subtotal'][] = $orproductpro->subtotal;
	$getpagarr[$orproductpro->order_id]['purchase_date'] = $orproductpro->billdate;
}
if(count($getpagarr)>0){
	/*echo '<pre>';
	print_r($getpagarr);*/?>

	<?php 
	$suntoto = 0;
	foreach($getpagarr as $key => $orproductpro){
		$suntoto = $suntoto + array_sum($orproductpro['subtotal']);
		}?>
	<span> Your Total earnings</span> <span style="color: #c09853; font-weight: bold;">$<?php echo $suntoto ;?></span>
	<table class="shop_table my_account_orders">
	<tr>
		<th class="order-number"><span class="nobr">Order#</span></th>
		<th class="order-number"><span class="nobr">Date</span></th>
		<th class="order-number"><span class="nobr">Buyer's Email</span></th>
		<th class="order-number"><span class="nobr">Total</span></th>
		<th class="order-actions">&nbsp;</th>
	</tr>
	<?php 
	foreach($getpagarr as $key => $orproductpro){
		//var_dump($orproductpro);?>
		<tr  class="order">
			<td class="order-number"><?php echo $key;?></td>
			<td class="order-date"><?php echo date_i18n( get_option( 'date_format' ), strtotime( $orproductpro['purchase_date'] ) ); ?></td>
			<td class="order-status" style="text-align:left; white-space:nowrap;"><?php echo $orproductpro['buyeremail'];?></td>
			<td class="order-total">$<?php echo array_sum($orproductpro['subtotal']);?> for <?php echo count($orproductpro['subtotal']); if(count($orproductpro['subtotal'])==1){echo ' item';}else{echo ' items';}?> </td>
			<td class="order-actions"><a href='<?php echo bloginfo ('url')."/my-account/view-order/?order=564|||".base64_encode(base64_encode($key));?>' class="button view">View</a></td>
		</tr>
		<?php 
	}?>
	</table>
