...
foreach($stylenamearr as $stylenamearrn){
	...
	print_r($rowf);echo '</pre>';*/
		...
		<input type="hidden" name="variation_id_<?php echo $styelnamvst['meta_value'];?>" value="<?php echo $rowf['ID'];?>" id="variation_id_<?php echo $styelnamvst['meta_value'];?>" />
		<?php 
...
"></div><?php echo apply_filters('single_add_to_cart_text', __( 'Buy it now', 'woocommerce' ), $product->product_type); ?></button>
			<?php }else{?>
			<button type="button" class="single_add_to_cart_button button alt" onclick="if($('.single_variation_wrap .single_variation .price .amount').length==0){$('#pa_style_1_price').html($('.price .amount').html());$('#pa_style_1_pricehidden').val($('.price .amount').html());}else{$('#pa_style_1_price').html($('.single_variation_wrap .single_variation .price .amount').html());$('#pa_style_1_pricehidden').val($('.single_variation_wrap .single_variation .price .amount').html());}$('#cgehckban').css('display','block'); $('#pa_style_1').val($('#variation_idv').val()+'===='+$('#pa_style').val()+'===='+$('.single_variation_wrap .single_variation .price .amount').html().replace('$','')); "><div class="left-arrow
"></div><?php echo apply_filters('single_add_to_cart_text', __( 'Buy it now', 'woocommerce' ), $product->product_type); ?></button>
<?php }?>
		</div>
	</div>
...
<div class="soldHding"><?php if(isset($orproduct_pro['tot_prod_qty']) && !empty($orproduct_pro['tot_prod_qty'])){$proqtytot = $orproduct_pro['tot_prod_qty'];}else{$proqtytot =0;}echo $proqtytot;?> sold towards goal of <?php echo $stockv[0]; ?></div>
<?php if($proqtytot<$stockv[0]){?>
				<div class="soldSubTxt">
					Don't worry, you won't be charged unless the goal is reached! <a href="<?php echo get_permalink( 70 ); ?>">Learn why</a>.
				</div>
<?php }?>
				<?php $perc = (($proqtytot/$stockv[0])*100);
				if(empty($perc)){$perc = '0.5';}else if($perc>100){$perc='100';}else{$perc = $perc;}
				if(($ecdtime[0]<time()) || (isset($eend) && !empty($eend) && ($eend=='yes'))){ $clsdf='pgbarouterInct';}else{$clsdf='pgbarouter';}?>
				<div class="<?php echo $clsdf;?>"><div class="pgbar" style="width: <?php echo $perc;?>%;"></div></div>
      ...
      <div class="popmanCnt">
		...
		<input type="hidden" id="typevalqt" name="typevalqt"/>
          ...
          <ul class="selSec" id="addvamulli">
            <li>
              <ul class="bdyCnt">
                <li><input name="qty" type="text" id="qty_1" value="1" class="qtcl" onkeyup="changepriceval(1);" /></li>
				<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
...
if(esc_attr( sanitize_title($name) )=='pa_style'){$datavi = $proct[esc_attr( $term->slug )]['variation_id'].'===='.esc_attr( $term->slug ).'===='.$proct[esc_attr( $term->slug )]['price'];}else{$datavi=esc_attr( $term->slug );}
	        ...
	        <?php endforeach;?>
                <li id="pa_style_1_price"></li> <input type="hidden" id="pa_style_1_pricehidden" />
              </ul>
            </li>          
          </ul>
          ...
          <div class="btmBtnSec">
            <div class="grnLnk alingnvbhyj">
            <div class="grnLnk">
            <div class="grnLnk">
              <a href="javascript:void(0)" onclick="addtocartpup();">
			  Checkout Now<!--<img src="<?php echo get_template_directory_uri(); ?>/images/chckOut.gif" alt="" />--></a>
...
<script>
	function appendsli(){
		var listr = '';
		listr += '<li class="mainli"><ul class="bdyCnt"><li><input name="qty" type="text" id="qty_'+$('#idb').val()+'" value="1" class="qtcl" onkeyup="changepriceval('+$('#idb').val()+');" /></li>';
				<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
		...
		listr += '<li <?php echo $sfdc;?>><select id="<?php echo esc_attr( sanitize_title($name) ); ?>_'+$('#idb').val()+'" name="attribute_<?php echo sanitize_title($name); ?>"  class="<?php echo $stych;?>" onchange="selvalm(\'<?php echo esc_attr( sanitize_title($name));?>\',\'<?php echo esc_attr( sanitize_title($name) ); ?>_'+$('#idb').val()+'\');">';
				...
				listr += '</li>';
	        <?php endforeach;?>
                listr += '<li class="pa_pricefirst_1" id="<?php echo esc_attr( sanitize_title($name) ); ?>_'+$('#idb').val()+'_price">';
				if($('.single_variation_wrap .single_variation .price .amount').length==0){
					listr += $('.price .amount').html();
					}else{
					listr += $('.single_variation_wrap .single_variation .price .amount').html();
					}
			listr += '</li><input type="hidden" id="<?php echo esc_attr( sanitize_title($name) ); ?>_'+$('#idb').val()+'_pricehidden" value="';
if($('.single_variation_wrap .single_variation .price .amount').length==0){
					listr += $('.price .amount').html();
					}else{
					listr += $('.single_variation_wrap .single_variation .price .amount').html();
...
listr += '" /></ul></li>';
	...
	function selvalm(txt,txt1){
		if(txt=='pa_style'){
var idbn = txt1.replace('pa_style_','');
			var spil = $('#'+txt1).val().split('====');
$('#'+txt1+'_pricehidden').val(spil[2]);
			$('#'+txt1+'_price').html('$'+(parseInt($('#qty_'+idbn ).val())*parseFloat($('#'+txt1+'_pricehidden').val().replace('$',''))).toFixed(2));

		}
		
	}
function changepriceval(txt){
$('#pa_style_'+txt+'_price').text('$'+(parseFloat($('#pa_style_'+txt+'_pricehidden').val().replace('$',''))*parseFloat($('#qty_'+txt).val())).toFixed(2));
//alert(parseInt($('#qty_'+txt).val()));
}
j(document).ready(function()	
{
	$('#pa_style').change(function(){//alert($('#variation_idv').val());
		$.post('<?php echo get_template_directory_uri(); ?>/changestyleimage.php', {vid:$('#variation_id_'+$(this).val()).val()}).done(function(data) {//alert(data);
				$('#productimage').html(data);
			});
	});
	j(document).on('click','#id-seeBack',function(){
			j('#dv_front').hide();
			j('#dv_back').show();
		});
		j(document).on('click','#id-seeFront',function(){
			j('#dv_back').hide();
			j('#dv_front').show();
		});
	});
