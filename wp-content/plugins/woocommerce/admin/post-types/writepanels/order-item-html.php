...
<tr class="item <?php if ( ! empty( $class ) ) echo $class; ?>" data-order_item_id="<?php echo $item_id; ?>">
	...
	//echo "select * from wp_postmeta where meta_key='_is_custom' AND meta_value='yes' AND post_id=".$item['product_id'];
	$yesn = $wpdb->get_results("select meta_value from wp_postmeta where meta_key='_is_custom' AND meta_value='yes' AND post_id=".$item['product_id']);
	if($yesn[0]->meta_value=='yes'){
		$queryval=$wpdb->get_results("select meta_value from wp_postmeta where meta_key='_manage_pic' and post_id=".$item['product_id']);?>
		<div class="fade pro_fade" style="display: none;" id="prod_img_in_lb<?php echo $item['product_id'];?>">
							<div class="login" style="display: block;">
								<div class="crsIcn" style="display: block;"><a href="javascript:login_close(<?php echo $item['product_id'];?>);">x</a></div>
						...
						<?php 
		echo("<a href='javascript: void(0)' onclick='login_now(".$item['product_id'].")'>View Image</a>");
	}else{
		if ( $_product ) : ?>
