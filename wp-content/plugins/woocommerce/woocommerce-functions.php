...
function woocommerce_template_redirect() {
	...
	elseif ( ! is_user_logged_in() && ( is_page( woocommerce_get_page_id( 'edit_address' ) ) || is_page( woocommerce_get_page_id( 'view_order' ) ) || is_page( woocommerce_get_page_id( 'change_password' ) ) ) ) {
		wp_redirect( get_permalink( woocommerce_get_page_id( 'myaccount' ) ).'/?p=account' );
		exit;
	}
...
function woocommerce_cancel_order() {
	...
	if ( isset($_GET['cancel_order']) && isset($_GET['order']) && isset($_GET['order_id']) ) :
		...
		if ( $order->id == $order_id && $order->order_key == $order_key && in_array( $order->status, array( 'processing' ) ) && $woocommerce->verify_nonce( 'cancel_order', '_GET' ) ) :
		...
		wp_safe_redirect( get_permalink( woocommerce_get_page_id( 'myaccount' ) ).'/?p=account' );
		exit;
