...
class WC_Gateway_Paypal extends WC_Payment_Gateway {
	...
	function process_payment( $order_id ) {
		...
		if ( ! $this->form_submission_method ) {
			...
			$paypal_args = $this->get_paypal_args( $order );
			/*echo '<pre>';print_r($paypal_args);die();*/
			$cancelurl = $paypal_args['cancel_return'];
			$retrnurl = $paypal_args['return'];
			...
			$opd = array();
			$dcv = 0;
			$orproduct_order_meta  = mysql_query("SELECT b.meta_value as prod_id,c.meta_value as prod_qty FROM wp_woocommerce_order_items as a, wp_woocommerce_order_itemmeta as b, wp_woocommerce_order_itemmeta as c, wp_term_relationships as d WHERE a.order_id=".$order_id." AND a.order_item_id=b.order_item_id AND b.meta_key='_product_id' AND a.order_item_id=c.order_item_id AND c.meta_key='_qty' AND a.order_id=d.object_id AND d.term_taxonomy_id!=12");
			while($orproduct_order_metaval=mysql_fetch_array($orproduct_order_meta)){
				$dcv = $dcv+$orproduct_order_metaval['prod_qty'];
				$opd[$orproduct_order_metaval['prod_id']]['qty'] = $dcv;
			}
$chvall = '';
$prod_idd = array();
foreach($opd as $key=>$pronarg){
	$orproduct_totqty  = mysql_fetch_array(mysql_query("SELECT sum(b.meta_value) as tot_prod_qty FROM wp_woocommerce_order_itemmeta as a, wp_woocommerce_order_itemmeta as b WHERE a.meta_value=".$key." AND a.meta_key='_product_id' AND a.order_item_id=b.order_item_id AND b.meta_key='_qty'"));
			$popqty = $orproduct_totqty['tot_prod_qty'] - $pronarg['qty'];
	$prostk  = mysql_fetch_array(mysql_query("SELECT meta_value FROM wp_postmeta WHERE meta_key='_stock' AND post_id=".$key));
...
//echo $chvall;
		...
		if(isset($chvall) && !empty($chvall) && ($chvall=='crowd')){
			session_start();
			/*echo '<pre>';
			print_r($order);die();*/
			//echo $order->order_total;
			$_SESSION['prodids'] = implode(',',$prod_idd);
			$_SESSION['CancelURL'] = $cancelurl;
			$_SESSION['MaxTotalAmountOfAllPayments'] = $order->order_total;
			$_SESSION['user_id'] = $order->user_id;
			$_SESSION['ReturnURL'] = $retrnurl;
			...
			$_SESSION['modetype'] = $modetype;
			return array(
				'result' 	=> 'success',
				'redirect'	=> get_site_url().'/paypal/templates/Preapproval.php'
			);
		}else{
			return array(
				'result' 	=> 'success',
