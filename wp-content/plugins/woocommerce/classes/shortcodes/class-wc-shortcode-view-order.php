...
$order_id1= base64_decode(base64_decode($order_id));
$orderval			= new WC_Order($order_id1 );
		...
		$statusval = get_term_by('slug', $orderval->status, 'shop_order_status');

		echo '<p class="order-info">'
		. sprintf( __( 'Order <mark class="order-number">%s</mark> made on <mark class="order-date">%s</mark>', 'woocommerce'), $orderval->get_order_number(), date_i18n( get_option( 'date_format' ), strtotime( $orderval->order_date ) ) )
		. '. ' . sprintf( __( 'Order status: <mark class="order-status">%s</mark>', 'woocommerce' ), __( $statusval->name, 'woocommerce' ) )
		. '.</p>';
